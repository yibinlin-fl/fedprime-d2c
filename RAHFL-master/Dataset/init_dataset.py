...
class CIFAR_C(Dataset):
    def __init__(self, root, dataidxs=None, train=True, transform=None, corrupt_rate=0):
        self.root = root
    ...
    def Construct_Participant_Dataset(self):
        ...
        data = np.load(os.path.join(data_path, "random_corrupt_{}.npy".format(self.corrupt_rate)))
        target = np.load(os.path.join(data_path, "labels.npy"))
        if self.dataidxs is not None:
            data = data[self.dataidxs]          #onejsd跟jsd得到的data一致
