...
\evidenceneeded{M1 multi-training-seed stability for the held-out map2 four-arm comparison. Seeds 1/2 are implemented but are not reported until Formal artifacts are returned and audited.}
...
The remaining table cells are deliberately marked pending. M1 and M2 are implemented Formal repetitions; M3 is an implemented second-private-task protocol that still requires benchmark-first execution; S1 is a bounded leave-one-operator-out taxonomy stress; S2 is a protocol-matched HFL context table; and O1 is a conditional JTT Formal. None should be described as launched, in progress, or successful until returned artifacts are audited. The paper does not add a new loss, communication mechanism, hierarchical PEW, or operator-level PEW in response to these gaps.
