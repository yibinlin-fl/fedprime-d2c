...
The experiments answer four questions: whether CLE creates binding-aligned behavior, whether that behavior is predominantly local-first, whether PEW+BER differs from generic ERM/group/tail-risk objectives, and whether DSA suppression persists when we change the binding map or communication base. We report DSA together with operator-grid accuracy and frozen task metrics: client-average accuracy (Avg), minimum client accuracy (Worst), worst valid class--operator-cell accuracy (WCCA), and the mean within-class accuracy range across operators (CFG; lower is better).
...
\subsection{Shared Validation: Binding Map and Communication Base}
...
The second communication-base experiment uses native single-view CE with a protocol-matched \textbf{FedDF-fidelity} adapter. Base and BER arms share initialization, private batches, public distillation, and training budget. Pooled DSA changes from \(0.136989\) to \(0.029012\), while operator-grid accuracy changes by \(-0.0400\) points and last-five Avg by \(-0.6660\) points. The frozen utility gate fails, and both arms fall below the pre-registered 20\% operator-grid learning floor. We use this experiment as supporting evidence that DSA suppression also appears under a second protocol-matched communication base; it does not support a cross-base utility gain.

\evidenceneeded{second private dataset; dataset and protocol pending.}

\subsection{Axis II: HFL Mechanism-Family Context}

Axis II asks a different question from the matched objective study: whether representative heterogeneous-FL communication and robustness mechanisms naturally remove CLE. The planned context includes Local/ERM, FedMD, FedProto, FedTGP, FedDF, KT-pFL, FCCL, RHFL, AugHFL, and RAHFL under a protocol-matched S2-v2 budget. These implementations provide mechanism-family context rather than official-recipe leaderboards; FedTGP, for example, is a protocol-matched core adapter. Existing benchmark and smoke outputs are engineering checks and are not used as scientific results.

\input{tables/hfl_context_baselines}

\evidenceneeded{S2-v2 Formal scientific metrics for the HFL mechanism-family context table.}
...
The internal manuscript keeps unresolved evidence visible. M1, the three-seed map2 objective comparison, is complete. M2 remains the multi-seed extension of the local-first factorial. M3 is the second-private-dataset slot, with both dataset and protocol pending. S1 is a bounded taxonomy-stress slot with protocol pending. S2-v2 has engineering benchmark/pairing work but no Formal scientific metrics in the current evidence package. O1 remains an optional JTT Formal only if the final paper makes an empirical JTT comparison.

\evidenceneeded{M2 local-first training seeds 1/2.}
