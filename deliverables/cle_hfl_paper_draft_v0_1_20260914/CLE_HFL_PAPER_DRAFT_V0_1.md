...
Let \(Y\) denote the task class and \(E\) the corruption environment. Standard corruption-robust learning often implicitly treats environment and class as approximately independent, e.g.,
...
with bindings that can vary across clients. In the earlier family-level controlled construction, the dominant family for client \(k\) and class \(c\) is written as

\[
d(k,c)=g_{(c+k)\bmod 4},
\]

and family sampling obeys

\[
P(E=g\mid k,c)=\frac{1-\gamma_{\mathrm{CLE}}}{4}
+\gamma_{\mathrm{CLE}}\mathbf 1\{g=d(k,c)\}.
\]

Thus \(\gamma_{\mathrm{CLE}}=0\) defines a no-entanglement control, while \(\gamma_{\mathrm{CLE}}=0.9\) creates a strong but not deterministic dominant binding. The current CLE-v2 Formal refines this design to a client-specific and class-specific **concrete operator map**, with both seen and unseen operator roles in evaluation.
...
The **Public Environment Witness (PEW)** is trained only on public CIFAR-100 carrier images with programmatically generated corruption supervision. The predefined environment taxonomy is
...
The primary shortcut metric is DSA. We also report operator-grid accuracy and frozen task-level utility metrics including Avg, Worst, WCCA, and CFG. **[EVIDENCE NEEDED: insert the repository’s formal definitions of WCCA and CFG before submission; this draft preserves only their reported values.]**
...
## 10.5 RQ5: Cross-Binding-Map Replication
...
## 10.6 RQ5: Shortcut Suppression on a Second Communication Base
...
We do not describe the FedDF-fidelity result as a lossless or uniformly successful plugin experiment.

## 10.7 RQ4: Held-Out Map2 Matched Controls
...
We compare three non-deployable or control grouping schemes under the same strong-CLE setting:
