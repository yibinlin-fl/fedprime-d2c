...
当前投稿候选按现实优先级记录为：`ECAI 2027 (CCF-B)`主目标，`ECML PKDD 2027
(CCF-B)`第二目标，`IJCNN 2027 (CCF-C)`稳妥备选，`UAI 2027 (CCF-B)`仅在进一步强化
DSA识别/统计理论后作为条件性候选；`AISTATS 2027 (CCF-C)`虽主题匹配，但2026-09-29摘要、
2026-10-06全文截止过近，不建议为赶会破坏实验纪律。已核验日期中，IJCNN 2027全文截止为
2027-01-31，ECAI 2027全文截止为2027-04-14；ECML PKDD/UAI 2027正式截稿日仍待官网发布。

用户现有两个OpenI账号，分别剩余约15和16个V100小时，并确认可选择相同V100、相同镜像和
相同数据集。S2可按完整实验臂跨账号拆分，账号身份本身不是科学变量；但必须冻结同一commit、
输入SHA256、镜像/依赖、GPU型号、training seed 0、初始化、partition、轮数、local batch预算、
公共batch与评价协议。严禁将同一arm的不同轮次跨账号拼接。两账号应各跑一个预注册的
`Local/ERM`重复锚点，仅用于平台漂移审计；最终表不得事后挑选较好锚点，且跨账号runtime不得
作为方法效率比较。

当前S2仍是单training-seed（seed 0），不是十臂三seed。40轮十臂乐观下限约78.2 V100小时，
两个账号合计31小时仍不足；拆账号只缩短并行墙钟时间，不减少总GPU小时。推荐优先等待实验室
