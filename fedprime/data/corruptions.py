...
# CLE-HFL v2 uses concrete operators as the atomic environment. Families are
# retained only as audit metadata; neither FedFalsify nor the training losses
# receive them.
CIFAR_C_CORE_CORRUPTIONS: tuple[str, ...] = (
    "gaussian_noise",
    "shot_noise",
    "impulse_noise",
    "defocus_blur",
...
CORRUPTION_OPERATOR_FAMILIES: dict[str, str] = {
...
# One held-out operator from every descriptive family. This is an evaluation
# split, not a method assumption. Alternative split seeds can be generated by
# the v2 preparation script.
DEFAULT_UNSEEN_CORRUPTIONS: tuple[str, ...] = (
    "impulse_noise",
    "zoom_blur",
    "fog",
    "pixelate",
)
DEFAULT_SEEN_CORRUPTIONS: tuple[str, ...] = tuple(
    name for name in CIFAR_C_CORE_CORRUPTIONS if name not in DEFAULT_UNSEEN_CORRUPTIONS
)
...
def _elastic_transform(arr: np.ndarray, severity: int, rng: np.random.Generator) -> np.ndarray:
    from scipy.ndimage import gaussian_filter, map_coordinates

    alpha = [1.0, 1.5, 2.0, 2.5, 3.0][severity - 1]
    sigma = [0.8, 0.9, 1.0, 1.1, 1.2][severity - 1]
    shape = arr.shape[:2]
    dx = gaussian_filter(rng.uniform(-1.0, 1.0, size=shape), sigma=sigma) * alpha
    dy = gaussian_filter(rng.uniform(-1.0, 1.0, size=shape), sigma=sigma) * alpha
    x, y = np.meshgrid(np.arange(shape[1]), np.arange(shape[0]))
    coordinates = (y + dy, x + dx)
    channels = [
        map_coordinates(arr[:, :, channel], coordinates, order=1, mode="reflect")
