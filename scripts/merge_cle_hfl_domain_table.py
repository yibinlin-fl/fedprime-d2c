...
from scripts.run_cle_hfl_context import ARMS as FULL_CONTEXT_ARMS  # noqa: E402
from scripts.run_cle_hfl_context import PRACTICAL_ARMS  # noqa: E402


ASYM_ROWS = ("asymhfl_erm", "asymhfl_pew_ber")
SUPPORTED_CONTEXT_ROWS = (PRACTICAL_ARMS, FULL_CONTEXT_ARMS)
FINAL_ROWS = (*FULL_CONTEXT_ARMS, *ASYM_ROWS)
...
def merge_domain_table(context_root: Path, asym_root: Path, output_dir: Path) -> dict:
    ...
    context_rows = tuple(context.get("rows", {}))
    if context_rows not in SUPPORTED_CONTEXT_ROWS:
        raise ValueError(f"Unexpected context row set/order: {context_rows}")
    final_rows = (*context_rows, *ASYM_ROWS)
    ...
    merged = {
        "protocol": "cle_hfl_domain_table_map2_v2",
        "context_profile": context.get("merge_profile", "full"),
        **expected,
        "partition_seed": 0,
        "binding_map_seed": 2,
        "evaluation_seed": 20260909,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Merge a practical/full HFL context table with two AsymHFL rows.")
