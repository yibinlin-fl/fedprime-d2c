...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="OpenI CLE K0-B v2 generic probe gate.")
    ...
    with tarfile.open(source, "r:gz") as handle:
        ...
        for member in members:
            ...
            if member.issym() or member.islnk():
                raise ValueError(f"Links are not allowed in K0-B input: {member.name}")
...
def package_outputs(output_dir: Path, mode: str) -> Path:
    archive = ROOT / "outputs" / f"cle_generic_probe_k0b_seed0_{mode}_outputs.tar.gz"
    archive.parent.mkdir(parents=True, exist_ok=True)
    with tarfile.open(archive, "w:gz") as handle:
        handle.add(output_dir, arcname=f"outputs/{output_dir.name}")
        spec = ROOT / "docs/experiments/current/CLE_GENERIC_PROBE_K0B_OPENI_ZH.md"
        if spec.is_file():
            handle.add(spec, arcname="docs/experiments/current/CLE_GENERIC_PROBE_K0B_OPENI_ZH.md")
    log(f"Wrote {archive}")
    return archive
...
def main() -> None:
    ...
    extraction_root = ROOT / "local_runs/cle_generic_probe_k0b_openi_input"
    ...
    experiment = f"cle_generic_probe_k0b_seed0_{args.mode}"
    ...
    log(f"===== Running CLE K0-B v2 {args.mode} =====")
    run(command, environment)

    run_manifest = {
        "protocol": "cle_generic_probe_k0b_openi_v2_20260901",
        "mode": args.mode,
        ...
        "optional_oracle_coverage_run": False,
    }
    ...
    if not args.no_upload:
        ...
        for path in (
            ...
            output_dir / "primary_artifact_manifest.json",
            output_dir / "metrics/per_client_metrics.csv",
            output_dir / "metrics/bootstrap.json",
            output_dir / "probe_banks/bank_a_manifest.json",
            output_dir / "probe_banks/bank_b_manifest.json",
        ):
            if path.is_file():
                upload_paths.append(path)
        log("===== Uploading K0-B v2 outputs through c2net =====")
        upload_outputs(context, upload_paths)
    log(f"===== CLE K0-B v2 {args.mode} complete =====")
