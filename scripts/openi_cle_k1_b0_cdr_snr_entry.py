...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="OpenI CLE K1-B0 CDR-SNR localization gate.")
...
def package_outputs(output_dir: Path, mode: str) -> Path:
    archive = ROOT / "outputs" / f"cle_k1_b0_cdr_snr_seed0_{mode}_outputs.tar.gz"
    archive.parent.mkdir(parents=True, exist_ok=True)
    with tarfile.open(archive, "w:gz") as handle:
        handle.add(output_dir, arcname=f"outputs/{output_dir.name}")
        spec = ROOT / "docs/experiments/current/CLE_K1_B0_CDR_SNR_OPENI_ZH.md"
        if spec.is_file():
            handle.add(spec, arcname="docs/experiments/current/CLE_K1_B0_CDR_SNR_OPENI_ZH.md")
    return archive
...
def main() -> None:
    ...
    extraction = ROOT / "local_runs/cle_k1_b0_openi_input"
    safe_extract(source, extraction)
    input_root = extraction / INPUT_DIRECTORY
    verify_manifest(input_root)
    output_dir = ROOT / "outputs" / f"cle_k1_b0_cdr_snr_seed0_{args.mode}"
    command = [
        sys.executable,
        "scripts/analyze_cle_k1_b0_cdr_snr.py",
        "--mode",
        args.mode,
        "--public-root",
    ...
    run_manifest = {
        "protocol": "cle_k1_b0_cdr_snr_openi_v1",
        "mode": args.mode,
        ...
        "evaluation_assets_passed_to_analyzer": False,
    }
    ...
    log(f"===== CLE K1-B0 {args.mode} complete =====")
