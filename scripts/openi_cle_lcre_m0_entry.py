...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="OpenI LCRE M0 cheap method gate")
    parser.add_argument("--mode", choices=("smoke", "benchmark", "formal"), default="benchmark")
    parser.add_argument("--data-source", default="")
    parser.add_argument("--device", default="auto")
    parser.add_argument("--confirm-formal", nargs="?", const=True, default=False, type=parse_bool)
    parser.add_argument("--skip-install", nargs="?", const=True, default=False, type=parse_bool)
    parser.add_argument("--no-upload", nargs="?", const=True, default=False, type=parse_bool)
...
def prepare_c2net():
    try:
        ...
        log(f"[warning] c2net unavailable: {exc}")
        return None
...
def candidate_roots(args: argparse.Namespace, context) -> list[Path]:
    ...
    for raw in (
        ...
        "/tmp/dataset", "/dataset", "/cache/dataset", "/tmp", "/cache",
    ):
        if raw:
            path = Path(raw)
...
def package(output_dir: Path, mode: str) -> Path:
    archive = ROOT / "outputs" / f"cle_lcre_m0_seed0_{mode}_outputs.tar.gz"
    archive.parent.mkdir(parents=True, exist_ok=True)
    with tarfile.open(archive, "w:gz", compresslevel=6) as handle:
        handle.add(output_dir, arcname=f"outputs/{output_dir.name}")
        for path in (
            ROOT / "configs/cle_lcre_m0_seed0.json",
            ROOT / "docs/experiments/current/CLE_LCRE_M0_CHEAP_METHOD_GATE_ZH.md",
        ):
...
def main() -> None:
    ...
    if args.mode == "formal" and not args.confirm_formal:
        raise ValueError("Formal is locked pending explicit user approval")
    os.chdir(ROOT)
    ...
    if source.stat().st_size != INPUT_BYTES or sha256_file(source) != INPUT_SHA256:
        raise ValueError("LCRE M0 inherited input archive bytes/hash mismatch")
    extraction = ROOT / f"local_runs/cle_lcre_m0_openi_extracted_{args.mode}"
    safe_extract(source, extraction, include_evaluation=args.mode == "formal")
    input_root = extraction / INPUT_DIRECTORY
    verified = verify_input(input_root, include_evaluation=args.mode == "formal")
    log(f"[integrity] verified {verified} non-public input files; public carriers not extracted")

    config = json.loads((ROOT / "configs/cle_lcre_m0_seed0.json").read_text(encoding="utf-8"))
    config["paths"] = {
    ...
    platform_config = extraction / "cle_lcre_m0_openi_config.json"
    platform_config.write_text(json.dumps(config, indent=2), encoding="utf-8")
    output_dir = ROOT / "outputs" / f"cle_lcre_m0_seed0_{args.mode}"
    command = [
        sys.executable, "scripts/run_cle_lcre_m0.py", "--mode", args.mode,
        "--config", str(platform_config), "--output-dir", str(output_dir), "--device", args.device,
    ]
    if args.mode == "formal":
        command.append("--confirm-formal")
    run(command, environment)
    run_manifest = {
        "protocol": "cle_lcre_m0_openi_v1",
        "mode": args.mode,
        "input_archive": source.name,
        "input_bytes": source.stat().st_size,
        "input_sha256": sha256_file(source),
        "public_carriers_extracted": False,
    ...
    log(f"===== LCRE M0 {args.mode} complete =====")
