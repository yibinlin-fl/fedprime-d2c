...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="OpenI Phase-B0 bridge-only kill test.")
    ...
    with tarfile.open(source, "r:gz") as handle:
        ...
        for member in members:
            ...
            if member.issym() or member.islnk():
                raise ValueError(f"Links are not allowed in Phase-B0 input: {member.name}")
...
def verify_manifest(input_root: Path) -> dict[str, object]:
    ...
    if manifest.get("protocol") != "cle_public_canonicalization_phase_b0_seed0_input":
        raise ValueError("Unexpected Phase-B0 input protocol")
    files = manifest.get("files")
    if not isinstance(files, list):
        raise TypeError("Phase-B0 manifest files must be a list")
    expected = {
    ...
    if recorded != expected:
        raise ValueError(f"Phase-B0 manifest file set mismatch: {sorted(recorded ^ expected)}")
    for row in files:
    ...
    log(f"[integrity] verified {len(files)} frozen input files")
    return manifest


def package_outputs(output_dir: Path, mode: str) -> Path:
    archive = ROOT / "outputs" / f"cle_public_canonicalization_phase_b0_seed0_{mode}_outputs.tar.gz"
    archive.parent.mkdir(parents=True, exist_ok=True)
    with tarfile.open(archive, "w:gz") as handle:
        handle.add(output_dir, arcname=f"outputs/{output_dir.name}")
        spec = ROOT / "docs/experiments/current/CLE_PUBLIC_CANONICALIZATION_PHASE_B0_ZH.md"
        if spec.is_file():
            handle.add(
                spec,
                arcname="docs/experiments/current/CLE_PUBLIC_CANONICALIZATION_PHASE_B0_ZH.md",
            )
    log(f"Wrote {archive}")
    return archive
...
def main() -> None:
    ...
    log(f"Phase-B0 input archive: {source}")
    if source.stat().st_size != INPUT_BYTES or sha256_file(source) != INPUT_SHA256:
        raise ValueError(
            f"Phase-B0 input archive mismatch: expected bytes={INPUT_BYTES}, "
            f"sha256={INPUT_SHA256}"
        )
    extraction_root = ROOT / "local_runs/cle_public_canonicalization_phase_b0_openi_input"
    ...
    experiment = f"cle_public_canonicalization_phase_b0_seed0_{args.mode}"
    output_dir = ROOT / "outputs" / experiment
    canonicalizer_dir = output_dir / "canonicalizer"
    ...
    analyze_command = [
        ...
        "--canonicalizer-checkpoint",
        str(canonicalizer_dir / "public_nuisance_canonicalizer.pt"),
        "--output-dir",
        str(analysis_dir),
        "--device",
        args.device,
    ...
    if args.mode == "smoke":
        train_command.extend(
            ["--smoke", "--epochs", "1", "--public-size", "256", "--batch-size", "128"]
        )
        analyze_command.extend(["--smoke", "--max-sources", "20"])

    log(f"===== Training public canonicalizer ({args.mode}) =====")
    run(train_command, environment)
    log(f"===== Auditing three bridges ({args.mode}) =====")
    run(analyze_command, environment)

    run_manifest = {
        "protocol": "cle_public_canonicalization_phase_b0_openi",
        "mode": args.mode,
    ...
    if not args.no_upload:
        ...
        for path in (
            canonicalizer_dir / "public_nuisance_canonicalizer_summary.json",
            analysis_dir / "cle_public_canonicalization_phase_b0_summary.json",
            analysis_dir / "cle_public_canonicalization_phase_b0_per_client.csv",
        ):
            if path.is_file():
                upload_paths.append(path)
        log("===== Uploading Phase-B0 outputs through c2net =====")
        upload_outputs(context, upload_paths)
    log(f"===== CLE Public Canonicalization Phase-B0 {args.mode} complete =====")
