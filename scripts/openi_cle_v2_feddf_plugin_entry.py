...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="OpenI CLE-v2 FedDF PEW+BER A/B.")
...
def main() -> None:
    ...
    if args.mode == "formal" and args.confirm_formal != "true":
        raise PermissionError("FedDF plugin Formal is locked")
    os.chdir(ROOT)
    ...
    package_root = safe_extract(source, ROOT / "local_runs/openi_cle_v2_feddf_plugin_input")
    if package_root.name != PACKAGE_DIRECTORY:
        raise ValueError(f"Unexpected extracted package: {package_root}")

    outputs_root = ROOT / f"outputs/cle_v2_feddf_plugin_seed0_{args.mode}"
    configs_root = ROOT / f"local_runs/cle_v2_feddf_plugin_seed0_{args.mode}_configs"
    analysis_root = ROOT / f"outputs/cle_v2_feddf_plugin_seed0_{args.mode}_analysis"
        ...
        json.dumps(
            {
                ...
                "warning": (
                    None
                    if args.mode == "formal"
                    else f"{args.mode} validates execution/cost only; metrics are not evidence."
                ),
            },
            indent=2,
        ),
    ...
    archive = package_light_outputs(
        ...
        configs_root,
        analysis_root,
        ROOT / f"cle_v2_feddf_plugin_seed0_{args.mode}_outputs.tar.gz",
    )
