...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="OpenI CLE-v2 mechanism Stage-1.")
...
def package_light_outputs(
    ...
    archive_path = archive_path or ROOT / f"cle_v2_mechanism_stage1_seed0_{mode}_outputs.tar.gz"
...
def main() -> None:
    ...
    if args.mode == "formal" and args.confirm_formal != "true":
        raise PermissionError("Stage-1 Formal is locked; explicit confirmation is required")
    os.chdir(ROOT)
    ...
    import_root = ROOT / "local_runs/openi_cle_v2_mechanism_stage1_input"
    ...
    outputs_root = ROOT / f"outputs/cle_v2_mechanism_stage1_seed0_{args.mode}"
    configs_root = ROOT / f"local_runs/cle_v2_mechanism_stage1_seed0_{args.mode}_configs"
    analysis_root = ROOT / f"outputs/cle_v2_mechanism_stage1_seed0_{args.mode}_analysis"
    ...
    if args.mode == "benchmark":
        analysis_command.extend(["--bootstrap-samples", "100", "--permutations", "100"])
    else:
        ...
        json.dumps(
            {
                ...
                "rounds": 1 if args.mode == "benchmark" else 12,
                "local_batches_per_client_round": 16,
                "scientific_evidence": args.mode == "formal",
                "benchmark_warning": (
                    "Benchmark validates execution and cost only."
                    if args.mode == "benchmark"
                    else None
                ),
            },
