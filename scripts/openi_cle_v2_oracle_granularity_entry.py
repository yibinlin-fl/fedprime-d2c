...
from scripts.openi_cle_v2_plugin_stage2_entry import (  # noqa: E402
    package_light_outputs,
    upload,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="OpenI CLE-v2 oracle granularity Kill Test.")
...
def main() -> None:
    ...
    if args.mode == "formal" and args.confirm_formal != "true":
        raise PermissionError("Oracle granularity Formal is locked")
    os.chdir(ROOT)
    ...
    package_root = safe_extract(source, ROOT / "local_runs/openi_cle_v2_oracle_granularity_input")
    if package_root.name != PACKAGE_DIRECTORY:
        raise ValueError(f"Unexpected extracted package: {package_root}")

    outputs_root = ROOT / f"outputs/cle_v2_oracle_granularity_seed0_{args.mode}"
    configs_root = ROOT / f"local_runs/cle_v2_oracle_granularity_seed0_{args.mode}_configs"
    analysis_root = ROOT / f"outputs/cle_v2_oracle_granularity_seed0_{args.mode}_analysis"
        ...
        json.dumps(
            {
                ...
                "local_batches_per_client_round": {"smoke": 1, "benchmark": 8, "formal": 16}[
                    args.mode
                ],
                ...
                "warning": (
                    None
                    if args.mode == "formal"
                    else f"{args.mode} validates execution/cost only and is not scientific evidence."
                ),
            },
            indent=2,
        ),
    ...
    archive = package_light_outputs(
        ...
        configs_root,
        analysis_root,
        ROOT / f"cle_v2_oracle_granularity_seed0_{args.mode}_outputs.tar.gz",
    )
