...
RESULT_ARCHIVE="${RESULT_ARCHIVE:-/kaggle/working/sara_residual_alpha05_results.tar.gz}"
CONFIG="${CONFIG:-configs/kaggle_t4_sara_residual_rahfl.yaml}"
DEBUG_CONFIG="${DEBUG_CONFIG:-configs/debug_sara_residual_rahfl.yaml}"

echo "===== SARA + receiver-side class residual AsymHFL launcher ====="
...
if [ "${RUN_DEBUG}" = "1" ]; then
  echo "===== Running residual debug smoke ====="
  PYTHONUNBUFFERED=1 "${PYTHON_BIN}" -u scripts/run_experiment.py --config "${DEBUG_CONFIG}"
fi

echo "===== Running SARA residual alpha=0.5 experiment ====="
