...
RESULT_ARCHIVE="${RESULT_ARCHIVE:-/kaggle/working/sara_rahfl_seed12_results.tar.gz}"
DEBUG_CONFIG="${DEBUG_CONFIG:-configs/debug_sara_local_only.yaml}"

CONFIGS=(
  "configs/kaggle_t4_sara_rahfl_seed1.yaml"
  "configs/kaggle_t4_sara_rahfl_seed2.yaml"
)

echo "===== SARA + AsymHFL seed1/seed2 Kaggle launcher ====="
...
if [ "${RUN_DEBUG}" = "1" ]; then
  echo "===== Running SARA debug smoke ====="
  PYTHONUNBUFFERED=1 "${PYTHON_BIN}" -u scripts/run_experiment.py --config "${DEBUG_CONFIG}"
fi

echo "===== Running SARA + AsymHFL seed1/seed2 experiments ====="
for cfg in "${CONFIGS[@]}"; do
  echo "===== Running ${cfg} ====="
