...
RESULT_ARCHIVE="${RESULT_ARCHIVE:-/kaggle/working/sara_vs_rahfl_alpha01_results.tar.gz}"
...
if [ -n "${PARTITION_SOURCE}" ]; then
  echo "===== Importing alpha=0.1 partition pack ====="
...
echo "===== Running RAHFL vs SARA alpha=0.1 experiments ====="
for cfg in "${CONFIGS[@]}"; do
  echo "===== Running ${cfg} ====="
